pipeline {
    agent any
    environment {
        TF_VAR_gcp_project = "qwiklabs-gcp-00-5d17ee69ad6e"
        TF_VAR_bucket = "tf-remote-state-student_04_7e3e893ce338-12974-17243"
        REPOSITORY = "https://github.com/lbajwa/sample-awx-project.git"
    ...
    stages {
        ...
        stage('Terraform Init') {
            steps {
                dir("terraform") {
                    withCredentials([file(credentialsId: 'gcp-svc-acctlaks', variable: 'GOOGLE_CLOUD_KEYFILE_JSON')]) {
        ...
        stage('Ansible Playbook') {
            steps {
                dir("ansible") {
                    withCredentials([sshUserPrivateKey(credentialsId: "SSH_KEY", keyFileVariable: "ANSIBLE_PRIVATE_KEY_FILE"), file(credentialsId: 'gcp-svc-acctlaks', variable: 'GOOGLE_CLOUD_KEYFILE_JSON')]) {
